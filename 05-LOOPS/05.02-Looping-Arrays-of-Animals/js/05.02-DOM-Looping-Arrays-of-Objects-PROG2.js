...
// get the iframe tag which plays the video
...
*/
const animal0 = animalsArr[0];
h2.textContent = animal0.name;
p.textContent = animal0.desc;
...
// display video for first animal, by setting iframe src:
iframe.src = `https://www.youtube.com/embed/${animal0.youTube}`;

// 8. Make sure the html page is using PROG.js and reload the page in the browser. We should have the video and description at right, and an empty box at left.
...
for(let i = 0; i < animalsArr.length; i++) {
    // save the current animal to a variable:
    const animal = animalsArr[i];
    // concat the image file name for that animal,
    // replacing any space w a hyphen and adding .jpg"
    let file = animal.name.replace(' ', '-') + '.jpg';
    // concat the image tag:
    let img = `<img src="images/${file}" 
                onclick="swapAnimal(${i})">`;
    // output the img tag to the animalPicsDiv box:
    animalPicsDiv.innerHTML += img;
}
...
function swapAnimal(i) {
    ...
    // get the clicked animal by index
    const animal = animalsArr[i];
    console.log(animal);

    // Output animal name and desc:
    h2.textContent = animal.name;
    p.textContent = animal.desc;

    // set iframe src to You Tube video:
    iframe.src = `https://www.youtube.com/embed/${animal.youTube}`;
...
// bonus: capitalize: American bison
let file = fullSrc.slice(0, fullSrc.lastIndexOf('.')).split('/').pop().replace('-', ' ');
let animal = file[0].toUpperCase() + file.slice(1);
console.log('animal: ', animal);

// above, step by step
let indexOfLastDot = fullSrc.lastIndexOf('.');
let fullSrcNoExt = fullSrc.slice(0,indexOfLastDot);
// split into array at the slash
let srcPathArr = fullSrcNoExt.split('/');
console.log('srcPathArr: ', srcPathArr);
// get just the last item in array, that being "american-bison"
animal = srcPathArr.pop();
console.log('popped animal: ', animal); // american-bison
// replace "-"" w " "
animal = animal.replace('-', ' ');
console.log('animal: ', animal);
// capitalize first word: American
animal = animal[0].toUpperCase() + animal.slice(1);
console.log('animal: ', animal); // American bison

// shorter way: just slice from last / to .
// the animal is between the last slash and the last dot so get the index of both
indexOfLastDot = fullSrc.lastIndexOf('.');
let indexOfLastSlash = fullSrc.lastIndexOf('/');
// slice from pt A to pt B
animal = fullSrc.slice(indexOfLastSlash+1,indexOfLastDot);
// replace dash w space:
animal = animal.replace('-', ' ');
console.log('animal: ', animal);
