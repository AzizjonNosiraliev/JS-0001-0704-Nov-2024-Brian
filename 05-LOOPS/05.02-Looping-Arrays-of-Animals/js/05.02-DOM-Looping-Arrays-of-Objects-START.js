...
for(let i = 0; i<animalsArr.length; i++) {
    ...
    //12. Make a file name from object name:
    let animalFileName = animalObj.name;
    //Regex way to replace all spaces in the name with "-"
    animalFileName = animalFileName.replace(/\s/g, "-");
    animalFileName = animalFileName.toLowerCase() + ".jpg";

    // 13. Set pic src to the picFile path:
    pic.src = `images/${animalFileName}`;

    //14. give the pic a name property equal to the animal name
    pic.index = i;
    
    console.log(i, animalsArr[i].name);
    //15. 
    pic. addEventListener('click', swapAnimal);
    // 17. Output the `img` tag. use parentElement.appendChild(childObj);
    animalPicsDiv.appendChild(pic);
}
function swapAnimal() {
    console.log(this.index);
    const obj = animalsArr[this.index];
    descP.textContent = obj.desc;
    h2.textContent = `${obj.name} - ${obj.region}`;
    h2.textContent += obj.herbivore ? "- Herbivore": "- Carnivore";
    iframe.src = `https://www.youtube.com/embed/${obj.youTube}`;
}
...
// func
    ...
    console.log('param obj:'); // the pic that got clicked to call the function
...
// incl file ext
// '.' // no file ext
console.log('animName split():');

// '/' 

console.log('animalName lastIndexOf():');
