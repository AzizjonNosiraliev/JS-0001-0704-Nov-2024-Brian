...
let quantity = 12;
quantity = 13; // works!
quantity = "enough"; // changing datatype works

// const for objects
/*
Unlike primitives, arrays declared with const can be modified--to an extent. Items can be changed, added or deleted, but the array object itself cannot be **transformed** to a different datatype. So, with const, once an array, always an array. Arrays are considered **mutable** because you can modify contents: add/remove/sort items, etc. But **mutable** does NOT also mean you can change array to other datatype, such as a string

To declare an array, start with let or const, followed by a name. We will start with let in order to later demonstrate our point about datatype. The array value consists of a pair square brackets containing items separated by commas.
...
// problem with let: it LETS you change datatype
// which we do not want to ever happen to our array
...
fruitsArr = ['peach', 'banana', 'cherry', 'mango'];
console.log(fruitsArr);
...
// 9. Try to change the array into a string. You get an error:
// fruits = "whoops"; // ERROR: Assignment to constant variable
...
// With fruits.length, we avoid hard-coding the index as we add more items. 
// But there's yet a better way: array.push(item)
...
console.log('poppedItem:', poppedItem); // pear
console.log(fruits);
...
// 25. Sort the nums array. Rather than being put in numeric order, the items are alphabetized -- all we get is a lousy "stringy number sort"
nums.sort();
...
[3, 5, 2, 17, 220, 105, 45, 65, 1008, 25];
*/

// 26. Call the sort method on the nums array, passing in the callback function. The result is an array of properly sorted numbers:
...
// [null, null, null, null, 'X', null, 'O', null, null]